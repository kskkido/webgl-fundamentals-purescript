module Pages.Shaders.Fudge.Main where
...
import Shaders.Fudge.Models.DrawConfig.Main as Shaders.Fudge.Models.DrawConfig
import Shaders.Fudge.Models.Context.Main as Shaders.Fudge.Models.Context
import Shaders.Fudge.Main as Shaders.Fudge
  ...
  Effect.Aff.launchAff_ do
    result <- ExceptT.runExceptT $ do
      ...
      ExceptT.mapExceptT Effect.Class.liftEffect do
        ...
        context <- Shaders.Fudge.Models.Context.fromEnv
          { canvas: canvas
          }
        controller <- ExceptT.lift $ do
          ...
          fudge <- do
            ...
            Lib.Element.setTextContent "fudge: " label
            pure $
              { field: field
              , input: input
              , value: value
        ...
        ExceptT.lift $ do
          subscribe <- pure $ ContT.runContT
            ( Lib.PubSub.merge
              ...
              , ( Lib.PubSub.fromEvent (Web.Event.Event.EventType "input") (Web.DOM.Element.toEventTarget controller.fudge.input) #
                  Lib.PubSub.filterMap Web.Event.Event.target #
                  Lib.PubSub.filterMap Web.HTML.HTMLInputElement.fromEventTarget #
                  Lib.PubSub.bindIO Web.HTML.HTMLInputElement.value #
                  Lib.PubSub.filterMap Number.fromString #
                  map (Tuple.Nested.tuple2 "fudge")
                )
              ...
              map Shaders.Fudge.Models.DrawConfig.fromMap
            )
          subscribe $ \config -> do
            Lib.Element.setTextContent (show config.count) controller.count.value
            Lib.Element.setTextContent (show config.fudge) controller.fudge.value
            ...
            result <- ExceptT.runExceptT $ Shaders.Fudge.main config context
