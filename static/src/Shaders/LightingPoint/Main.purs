module Shaders.LightingPoint.Main where
...
import Shaders.LightingPoint.Models.Context.Main as Context
import Shaders.LightingPoint.Models.DrawConfig.Main as DrawConfig
import Shaders.LightingPoint.Models.Env.Main as Env
...
main config context = do
  ...
  Lib.WebGL.setArrayBuffer context.buffers.normal context.webgl
  Lib.WebGL.setAttributeConfig context.attributes.normal context.webgl
  flip Traversable.traverse_ matrices $ \matrix -> do
    let mx = matrix # Lib.Array.transpose # Array.concat
        cx = camera # Lib.Array.transpose # Array.concat
    Lib.WebGL.uniformMatrix4fv false mx context.uniforms.matrix context.webgl
    Lib.WebGL.uniformMatrix4fv false cx context.uniforms.camera context.webgl
    Lib.WebGL.uniform3fv [config.light.x, config.light.y, config.light.z] context.uniforms.reverseLightDirection context.webgl
