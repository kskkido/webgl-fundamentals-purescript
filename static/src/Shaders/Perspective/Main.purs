module Shaders.Perspective.Main where
...
import Shaders.Perspective.Models.DrawConfig.Main as DrawConfig
import Shaders.Perspective.Models.Context.Main as Context
import Shaders.Perspective.Models.Env.Main as Env
...
main config context = do
  ...
  matrices <- do
    pure
      ( Array.replicate config.count
        ( [ Lib.Matrix4.translate $ Tuple.Nested.tuple3
              (Lib.Math.clamp config.position.x (-dimensions.canvas.x) dimensions.canvas.x)
              (Lib.Math.clamp config.position.y (-dimensions.canvas.y) dimensions.canvas.y)
              (Lib.Math.clamp config.position.z (-dimensions.canvas.z) dimensions.canvas.z)
